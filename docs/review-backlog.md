...
|---|-------|-------|-----|--------|-------|
| C1 | **Enable GitHub Private Vulnerability Reporting** | Repo Settings → Security → Private vulnerability reporting | Toggle ON, then run `./scripts/release-security-pvr.sh --confirm-enabled --screenshot <reference>`. Close `security-pvr` only after generated evidence is recorded in `docs/release/MANUAL_VALIDATION.md` and the matching `RELEASE_CHECKLIST.md` item is checked. | [ ] | Requires repo owner/admin access |
...
| C7 | **Windows PowerShell path never validated** | `bin/lac.ps1`, `runtime-config/launch/*.ps1` | On a Windows machine (or VM), run `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/release-windows-powershell.ps1 -FullRuntime`. Record generated `state/release-evidence/` output in `docs/release/MANUAL_VALIDATION.md` before closing the gate. | [ ] | Deferred per `STATE.md`; keep the gate open until real Windows evidence exists |
| C8 | **Fresh-clone validation incomplete** | `docs/release/MANUAL_VALIDATION.md`, `RELEASE_CHECKLIST.md` | Run `./scripts/release-fresh-clone-unix.sh --full-runtime` from a clean macOS or Linux clone. Close only after generated evidence is recorded and the matching checklist item is checked. | [ ] | Do NOT commit the test clone |
...
| M7 | **OpenRouter free model sync is manual** | `docs/providers/OPENROUTER_FREE.md` and `docs/free-coding-models.json` | Add a CI check (not a cron) that warns if these files are older than 14 days. Or add a `make refresh-catalogs` target that runs `./scripts/sync-free-cloud-models.sh`. | [x] | Done in this session — `scripts/check-provider-doc-freshness.sh` warns in CI/local beta without auto-refreshing or committing catalog noise |
...
| M9 | **`lac.py` is 2397 lines with no module structure** | `scripts/lac.py` | Consider extracting subcommands into `scripts/lac/commands/` (e.g., `profile.py`, `runtime.py`, `doctor.py`). Not required for beta, but note as technical debt. | [x] | Deferred to post-beta technical debt — `scripts/lac.py` is now 2642 lines, but splitting the CLI before beta would be high-blast-radius and is tracked in `docs/release/STATE.md` instead |
...
| # | Issue | Where | Fix | Status | Notes |
|---|-------|-------|-----|--------|-------|
| L1 | **Mixed script invocation styles** | Some scripts use `python3 - <<'PY'`, others call `python3 script.py` directly | Standardize: either all inline Python or all standalone scripts. `lac.py` is fine as the main CLI. Verify scripts can stay as `python3 - <<'PY'` for portability. | [x] | Done in this session — CONTRIBUTING now standardizes public helper entrypoints as shell/PowerShell wrappers with inline Python for small validators, while reserving standalone Python for `scripts/lac.py` and `scripts/lib/` |
| L2 | **Missing `docs/providers/CODEX_AUTH.md` cross-links** | `docs/providers/CODEX_AUTH.md` | Add a "See also" section linking to `docs/providers/AUTHENTICATION.md` and `docs/use-cases/ONBOARDING_CLAUDE_CODE.md`. | [x] | Done in this session — added provider/auth and Claude Code onboarding cross-links |
| L3 | **`docs/release/STATE.md` completion markers** | Several items under "Completed" were checked but the "Open Questions" section still has unchecked items from older phases | If an open question is resolved, move it to "Completed". If still open, add a target date or owner. | [x] | Done in this session — open `STATE.md` questions now carry explicit owners and target conditions, with verifier coverage preventing ownerless/targetless bullets |
| L4 | **No `opencode.template.jsonc` schema validation** | The template is JSONC with comments | Add a CI step that validates JSONC syntax (strip comments, `json.loads()`). `verify-config-schema.sh` already does this — verify it runs on `opencode.template.jsonc` (yes, line 13 lists it). | [x] | Done in this session — `scripts/verify-config-schema.sh` validates `opencode.template.jsonc`, and `.github/workflows/ci.yml` plus `scripts/verify-public-beta-local.sh` run it with documentation verifier coverage |
| L5 | **`catalog/workflow-packs.json` not referenced** | File exists but no script or doc references it | Either: document it in `docs/use-cases/SCENARIO_GUIDE.md` as the source of pack definitions, or remove it if unused. | [x] | Done in this session — scenario guide now points to the canonical workflow-pack catalog and lists public pack labels |
...
- [x] H4 — Document profile verification tiers in `docs/model-recommendations.md`
