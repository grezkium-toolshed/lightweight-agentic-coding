...
| `fresh-clone-unix` | open | release tester | Fresh macOS or Linux clone runs `python3 -m pip install .`, `lac init --yes --profile 24gb --no-cloud`, `lac models sync 24gb`, `lac runtime start`, and `lac client render opencode`. Attach date, OS, Python version, lac version, and command transcript summary. |
