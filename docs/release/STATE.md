...
### Open Questions
- [ ] Owner: repo owner; Target condition: close `security-pvr` only after GitHub Private Vulnerability Reporting is enabled in repo settings and `./scripts/release-security-pvr.sh --confirm-enabled --screenshot <reference>` evidence is recorded, including generated `state/release-evidence/` output, GitHub docs/settings path, `gh repo view` metadata, security policy URL, `SECURITY.md` wording check, repo owner/admin confirmation, date enabled, and screenshot/reference from repository Settings > Advanced Security showing Private vulnerability reporting enabled
- [ ] Owner: 128GB Apple Silicon tester; Target condition: close `ds4-128gb` only after manual 128GB-class Apple Silicon validation runs `./scripts/release-ds4-128gb.sh --full-runtime` and records generated `state/release-evidence/` output, hardware/RAM/macOS details, ds4 binary path/version or commit, `lac profile apply 128gb-ds4-flash --json`, runtime status showing ds4 paths and port 8000, `lac models sync 128gb-ds4-flash`, `curl http://127.0.0.1:8000/v1/models`, and OpenCode ds4 model selection/session notes
- [ ] Owner: Windows tester; Target condition: close `windows-powershell` only after Windows PowerShell fresh-clone validation runs `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/release-windows-powershell.ps1 -FullRuntime` and records generated `state/release-evidence/` output, Windows OS details, PowerShell version, Python version/path, git commit, `bin/lac.ps1 --version`, `bin/lac.ps1 init --yes --profile 24gb --no-cloud --json`, `bin/lac.ps1 doctor --bootstrap-hint --json`, `bin/lac.ps1 smoke --json`, transcript/summary path, and model sync/runtime start/status evidence when captured by the helper
- [ ] Owner: release tester; Target condition: close `opencode-discovery` only after a real OpenCode discovery validation runs `./scripts/release-opencode-discovery.sh --open` and records generated `state/release-evidence/` output plus screenshot, transcript, or manual session notes
- [ ] Owner: maintainer; Target condition: close `provider-live-freshness` only after live provider freshness probes run with release credentials via `./scripts/release-provider-freshness.sh --refresh-catalog`, or every skipped provider has a documented release skip reason with generated evidence recorded
- [ ] Owner: maintainer; Target condition: close `linux-ci` only after Linux CI is green on the release branch with `./scripts/release-linux-ci.sh --run-id <run-id>` evidence, including generated `state/release-evidence/` output, workflow URL, commit SHA, selected run ID, branch, workflow name, run status/conclusion, and `.github/workflows/ci.yml` coverage confirmation
...
- [x] Linux CI helper documentation wired: `scripts/release-linux-ci.sh --allow-unavailable` is documented as automated/local rehearsal, while `--run-id <run-id>` plus generated `state/release-evidence/` output, workflow URL, commit SHA, selected run ID, branch, workflow name, run status/conclusion, and `.github/workflows/ci.yml` coverage confirmation remain required to close the gate
- [x] Historical plan/backlog docs labelled as superseded before public beta, and `verify-documentation.sh` now requires old plans to point readers at current release gates rather than stale implementation instructions
- [x] Open release questions now name an owner and target condition, and `verify-documentation.sh` prevents ownerless or targetless public-beta blockers from returning
- [x] OpenCode template schema validation coverage confirmed: `scripts/verify-config-schema.sh` validates `opencode.template.jsonc` and `templates/opencode/opencode.example.jsonc`, and both `.github/workflows/ci.yml` and `scripts/verify-public-beta-local.sh` run it
- [x] Script style guidance documented for public-beta contributors: helper entrypoints stay as shell/PowerShell wrappers, small validators may use inline Python, and standalone Python remains limited to `scripts/lac.py` plus shared `scripts/lib/` modules
- [x] Manual release handoff helper now lists exact evidence-helper commands and exposes each open gate command bundle in JSON for tester handoff
- [x] Public navigation pass completed: Codex auth docs link to provider onboarding and Claude Code guidance, `docs/use-cases/SCENARIO_GUIDE.md` points to `catalog/workflow-packs.json`, and release/local documentation checks enforce those links
- [x] Provider/free-model freshness age warning added: `scripts/check-provider-doc-freshness.sh` warns in CI and `scripts/verify-public-beta-local.sh` when OpenRouter/free-model freshness markers are older than the release threshold, without auto-refreshing live catalogs
- [x] Provider freshness release evidence now writes explicit `provider-skip-reasons.json` via `--skip-reason <provider-id=reason>` or `--skip-reasons-file <path>`, so intentional release skips can be reviewed without secrets or loose prose
- [x] Hardware validation scope resolved for public beta: release-blocking live proof is limited to the representative `24gb` Unix fresh-clone gate, Windows PowerShell wrapper gate, llama.cpp smoke gate, OpenCode discovery gate, and explicit `128gb-ds4-flash` gate; `16gb`, `macos-16gb`, `32gb`, `64gb`, `gemma-*`, `128gb-multi`, `128gb-qwen122b`, and `128gb-minimax` stay covered by automated profile/preset/schema/package checks and verification tiers, but are not marketed as hardware-validated until separate evidence exists
...
- Live hardware validation for non-gating local profiles (`16gb`, `macos-16gb`, `32gb`, `64gb`, `gemma-*`, `128gb-multi`, `128gb-qwen122b`, `128gb-minimax`); public beta should describe these through verification tiers and local self-validation until separate hardware evidence is captured
