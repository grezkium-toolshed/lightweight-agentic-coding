...
- [ ] Linux CI green on release branch with `./scripts/release-linux-ci.sh --run-id <run-id>` evidence, including generated `state/release-evidence/` output, workflow URL, commit SHA, selected run ID, branch, workflow name, run status/conclusion, and `.github/workflows/ci.yml` coverage confirmation
